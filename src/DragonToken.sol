...
contract DragonFire is ERC20, ERC20Permit, Ownable {
    ...
    event ProcessFees(address indexed user, uint256 dragonTokensProcessed);

    event SettingsChanged(address indexed user, string indexed setting); // This is to alert the public that an owner setting has changed, rather than track all the particulars of the setting changed

    event IncludeInFees(address indexed userIncluded);

    event ExcludeFromFees(address indexed userExcluded);
    ...
    event NFTWithdraw(address indexed to, uint256 ID, address indexed token);

    modifier feeSettingsLock() {
        require(
            tradingPhase() == TOTAL_PHASES,
    ...
    constructor() ERC20("Test", "TST") ERC20Permit("Test") Ownable(msg.sender) {
        ...
        communityTokens = // FUJI Testnet Chainlink: 0x0b9d5D9136855f6FEc3c0993feE6E9CE8a297846
        // [
        ...
        require(
            length_ > 0,
            "Contract must have at least one community token in rewardsToken array"
        );

        // Choose dex router with best CT/AVAX liquidity for each CT
        ctRouters = // Fuji Testnet: 0xd7f655E3376cE2D7A2b08fF01Eb3B1023191A901
        // [
        ...
        for (uint256 i = 0; i < length_; i++) {
            ...
            uniswapV2Pair_ = IUniswapV2Factory(uniswapV2Routers[i].factory())
                .getPair(communityTokens[i], WAVAX);
            require(
                uniswapV2Pair_ != address(0),
                "All CT/WAVAX LP Pairs must be created first and have some LP already, to buy CT with AVAX"
            );
        }
    ...
    function setMainDex(address router_) external onlyOwner feeSettingsLock {
        ...
        uniswapV2Router = uniswapV2Router_; // Uses the interface created above
        address uniswapV2Pair_ = IUniswapV2Factory(uniswapV2Router.factory())
            .getPair(address(this), WAVAX); // This pair must already exist

        require(
            uniswapV2Pair_ != address(0),
            "LP Pair must be created first, paired with WAVAX"
        ); // An invalid LP pair will return 0 address.
    ...
    function setCtRouters(
        address[] memory routers_
    ) public onlyOwner feeSettingsLock {
        ...
        for (uint256 i = 0; i < length_; i++) {
            ...
            require(
                uniswapV2Pair_ != address(0),
                "All CT/WAVAX LP Pairs must be created first on new dex, to buy CT with AVAX"
            );
        }
    ...
    function setProcessFeesMinimum(
        uint256 amount_
    ) external onlyOwner feeSettingsLock {
        // Set minimum amount of Dragon tokens collected as fees to swap and burn with
        require(
            amount_ >= TOTAL_SUPPLY_WEI / 1000000000,
            "Amount too low, cannot be less than 0.0000001% of supply"
        );
        require(
            amount_ <= TOTAL_SUPPLY_WEI / 1000,
            "Amount too high, cannot be more than 0.1% of supply"
        );
    ...
    function setFeesInBasisPts(
        // Set transfer fees in basis points where 100 = 1%
    ...
    function setPairs() external onlyOwner {
        ...
        for (uint256 i = 0; i < length_; i++) {
            uniswapV2Pair_ = IUniswapV2Factory(uniswapV2Router.factory())
                .getPair(address(this), communityTokens[i]);
            if (
                !dragonCtPairs[uniswapV2Pair_] && uniswapV2Pair_ != address(0)
            ) {
    ...
    // Internal functions:
    function _update(
        // Fees added on to all transfers, and phases check
        address from_,
        address to_,
        uint256 amount_
        ...
        if (swapping) {
            ...
            revert(
                "User cannot trade in the middle of internal LP creation. You have created a reentrancy issue"
            );
        }
        ...
        if (
            tradingPhase() == 0 ||
            isExcludedFromFees[from_] ||
            isExcludedFromFees[to_]
        ) {
            // Don't charge fees when adding LP before launch
            takeFee_ = false;
        }
    ...
    function beforeTokenTransfer(address to_, uint256 amount_) private {
        ...
        if (to_ != uniswapV2Pair && !dragonCtPairs[to_]) {
            ...
            require(
                allowlisted[to_] <= tradingPhase_,
                "Not allowlisted for current phase"
            );
            totalPurchased[to_] += amount_; // Total amount user received in whale limited phases
            require(
                totalPurchased[to_] <= maxWeiPerPhase[tradingPhase_],
                "Receiving too much for current whale limited phase"
            );
        }
    }

    function swapAndBurnLP(
        uint256 swapToDRAGONLP_,
        uint256 swapPerCtLP_
    ) private {
        ...
        uint256 totalDRAGONTokens_ = (swapToDRAGONLP_ +
            (swapPerCtLP_ * ctLength_)); // Total DRAGON tokens to process here
        ...
        if (swapToDRAGONLP_ > 0) {
            uint256 dragonLpAVAX_ = (swapToDRAGONLP_ * newAVAXBalance_) /
                totalDRAGONTokens_; // Calculate ratio and amount of AVAX to be used for DRAGON/WAVAX LP.
        ...
        if (swapPerCtLP_ > 0) {
            ...
            for (uint256 i = 0; i < ctLength_; i++) {
                // Loop for each CT
                uint256 initialCtBalance_ = IERC20(communityTokens[i])
                    .balanceOf(address(this));
                swapAvaxForCT(avaxPerCt_, i);
                uint256 newCtBalance_ = IERC20(communityTokens[i]).balanceOf(
                    address(this)
                ) - initialCtBalance_;
                addLiquidityCT(
                    dragonPerCtLP_,
    ...
    function addLiquidityCT(
        uint256 dragonAmount_,
        uint256 ctAmount_,
        address ctAddress_
    ) private {
    ...
    function processFees() public {
        ...
        if (tradingPhase() == TOTAL_PHASES) {
            ...
            require(
                block.timestamp >= (lastTimeCalled + SECONDS_PER_PHASE),
                "Cannot process fees more than once every 8 minutes"
            ); // Prevent back to back fees dumping
        ...
        if (canSwap_ && !swapping) {
            ...
            uint256 swapToDRAGONLP_ = (amountOfFees_ * liquidityFee) /
            ...
            uint256 remainder_ = amountOfFees_ -
                (swapToDRAGONLP_ +
                    farmTokens_ +
                    treasuryTokens_ +
                    (swapPerCtLP_ * communityTokens.length)); // Calculate dust leftover
            ...
            emit ProcessFees(
                msg.sender,
                (contractDRAGONBalance_ - newDRAGONBalance_)
            ); // Amount of DRAGON fees processed minus any dust leftover
        } else {
            if (swapping) {
                revert(
                    "Cannot process now, already in the middle of processing fees. You have created a reentrancy issue"
                );
            }
            ...
            if (!canSwap_ && tradingPhase() == TOTAL_PHASES) {
                ...
                revert(
                    "Cannot process yet, more fees need to be collected first"
                );
            }
        }
    }
    ...
    function seedAndBurnCtLP(
        ...
        require(
            communityTokens[ctIndex_] == communityToken_,
            "Token not found in communityTokens array at that index"
        ); // Verify valid CT address
        require(
            amountDragon_ > 100000000000000000,
            "Must send at least 0.1 Dragon tokens"
        ); // @audit should be >=
        require(
            amountCt_ > 100000000000000000,
            "Must send at least 0.1 Community tokens"
        );
        require(
            !swapping,
            "Already making CT LP, you have created a reentrancy issue"
        );
        swapping = true; // Reentrancy block
        transfer(address(this), amountDragon_);
        IERC20(communityToken_).transferFrom(
            msg.sender,
            address(this),
            amountCt_
        ); // User must first approve this contract to spend their CT tokens
    ...
    function seedAndBurnDragonLP(
        ...
        require(tradingPhase() != TOTAL_PHASES, "Phases are completed already"); // This function is just to seed LP for IDO launch
        require(
            msg.value == amountAvax_,
            "Different amount of Avax sent than indicated in call values"
        ); // Verify intended amount sent
        require(
            amountDragon_ >= 100000000000000000,
            "Must send at least 0.1 Dragon tokens"
        );
        require(
            amountAvax_ >= 100000000000000000,
            "Must send at least 0.1 AVAX"
        );
        require(
            !swapping,
            "Already making Dragon LP, you have created a reentrancy issue"
        );
    ...
    function tradingRestricted() public view returns (bool) {
        ...
        return
            tradingActive() &&
            block.timestamp <=
            (startTime + (SECONDS_PER_PHASE * (TOTAL_PHASES - 1))); // True if tradingActive, but whale limited phases is not over
    }
    ...
    function setPhasesStartTime(
        uint256 startTime_
    ) external onlyOwner phasesLock {
    ...
    function setWhaleLimitsPerPhase(
        uint256[] memory maxWei_
    ) external onlyOwner phasesLock {
        uint256 length_ = maxWei_.length;
        require(
            length_ == (TOTAL_PHASES - 1),
        ...
        for (uint256 i = 0; i < length_; i++) {
            require(
                maxWei_[i] >= previousPhaseMaxWei_,
    ...
    function lockPhasesSettings() external onlyOwner phasesLock {
        ...
        require(
            ...
            "startTime must be set for the future"
        );
        require(
            maxWeiPerPhase[1] > 0,
    ...
    function setAllowlistedForSomePhase(
        ...
        require(
            length_ <= 200,
            "Cannot add more than 200 users in one transaction"
        ); // Prevent out of gas errors
    ...
    function noDRAGONTokens(address contract_) internal view {
        require(
            ...
            "Owner cannot withdraw $DRAGON token fees collected"
        ); // No $DRAGON tokens, as they are earmarked for fees processing
